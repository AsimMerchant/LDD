...
static long char_drv_ioctl(struct file *filep, unsigned int cmd, unsigned long arg)
{
	int err;
	...
	if(cmd==READ_CMD)
		copy_to_user((void *)arg,&readMe,sizeof(readMe));
